...
public class UserRepo {
    ...
    public void add_employee(Employee e) {
        String sql = "insert into employee(person_id,salary,username) values(?,?,?)";
        jdbcTemplate.update(sql, e.getPerson_id(), e.getSalary(), e.getUsername());
    }

    public void update_employee(Employee e) {
        String sql = "update  employee set salary=? where employee_id=?";
        jdbcTemplate.update(sql, e.getSalary(), e.getEmployee_id());
...
//    //-----------------------------------------------------------feedback--------------------------------------
    public void add_Feedback(Feedback f) {
        String sql = "insert into feedback(customer_id,description,item_id,rating,rid) values(?,?,?,?,?)";
        System.out.println(f.getCustomer_id());
        jdbcTemplate.update(sql, f.getCustomer_id(), f.getDescription(), f.getItem_id(), f.getRating(),f.getRid());
    }
    ...
    public void add_Income_Tax(Income_tax i) {
        String sql = "insert into income_tax(amount,penality,fixed_assets,loans_and_advances,other_assets,balance_with_banks,cash_in_hand,inventory) values(?,?,?,?,?,?,?,?)";
        jdbcTemplate.update(sql, i.getAmount(), i.getPenality(),i.getFixed_assets(),i.getLoans_and_advances(),i.getOther_assets(),i.getBalance_with_banks(),i.getCash_in_hand(),i.getInventory());
    }
    ...
    public void update_Tax(Income_tax i) {
        String sql = "update income_tax set amount=? , penality=?, fixed_assets =?, loans_and_advances=? , other_assets=? , balance_with_banks=? ,cash_in_hand=?,inventory=? where income_tax_id=?";

        jdbcTemplate.update(sql, i.getAmount(), i.getPenality(),i.getFixed_assets(),i.getLoans_and_advances(),i.getOther_assets(),i.getBalance_with_banks(),i.getCash_in_hand(),i.getInventory(),i.getIncome_tax_id());
    }
    ...
    public void add_User(User u) {
        String sql = "insert into user(username,active,password,recovery_code,type,role) values(?,?,?,?,?,?)";
        jdbcTemplate.update(sql, u.getUsername(), u.getActive(), u.getPassword(), u.getRecovery_code(), u.getType(),u.getRole());
    }
...
//        jdbcTemplate.update(sql,r.getAmount(),r.getAmount_payed(),r.getCustomer_id(),r.getDiscount());
    ...
    public long add_invoice(Invoice i) {
        String sql = "insert into invoice(amount,supplierid) values(?,?)";
        GeneratedKeyHolder keyHolder = new GeneratedKeyHolder();

        // the name of the generated column (you can track more than one column)
        String id_column = "invoice_id";

        // the update method takes an implementation of PreparedStatementCreator which could be a lambda
        jdbcTemplate.update(con -> {
                    PreparedStatement ps = con.prepareStatement(sql, new String[]{id_column});
                    ps.setInt(1, i.getAmount());
                    ps.setInt(2, i.getSupplierid());
                    return ps;
                }
                , keyHolder);
...
//        sql += " and description like '%" + f.getName() + "%'";
    ...
    public List<Employee> get_all_emp_by_filter(EmpFilter f) {
        ...
        sql += " and gender='" + f.getP().getGender() + "'";
    ...
    public void update_item(Item x) {
        String sql = "update  item set qt_avail=?, mrp=?, discount=?,description=?, sold_qt=?, rating=?, name=?, imgurl=?, cost_price=?, category=?, brand_id=? where item_id=?";
        jdbcTemplate.update(sql,x.getQt_avail(), x.getMrp(), x.getDiscount(), x.getDescription(), x.getSold_qt(), x.getRating(), x.getName(), x.getImgurl(), x.getCost_price(), x.getCategory(), x.getBrand_id(), x.getItem_id());
    }

    public List<Ritem> get_rc_items(long receipt_id) {
        String  sql="select * from ritem where rid="+receipt_id;
        System.out.println(sql);
        return jdbcTemplate.query(sql,new RcItemRowMapper());
    }


    public List<Reciept> get_reciept() {
        String sql = "select * from reciept";
        return jdbcTemplate.query(sql, new RcRowMapper());
    }

    public void update_user(User u) {
        String sql="update user set active=?,password=?,type=? where username=?";
        jdbcTemplate.update(sql,u.getActive(),u.getPassword(),u.getType(),u.getUsername());
    }

    public void add_invoice_item(Invoice i) {
        String  sql="insert into iitem values(?,?,?,?)";
        jdbcTemplate.update(sql,i.getInvoice_id(),i.getA().getItem_id(),i.getA().getCost_price(),i.getA().getQt_avail());

        // jdbcTemplate.update(sql,i.getInvoice_id(),i.getB().getItem_id(),i.getB().getCost_price(),i.getB().getQt_avail());
    }

    public List<IItem> get_invoce_item(int id) {
        String sql="select * from iitem where iid="+id;
        return jdbcTemplate.query(sql,new IItemRowMapper());
