...
import com.example.demo.auth.RandomGenerator;
...
//        System.out.println(f.getE().getSalary());
    ...
    public ModelAndView get_item(@PathVariable int id) {
        ...
        ModelAndView m = new ModelAndView("/item/detail.jsp", "item", i);

        String type = get_user().getType();

        m.addObject(type, type);
        m.addObject("ff",userRepository.get_all_Feedback_by_Item(id));
        return m;
    }
...
//Supplier Begin-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public ModelAndView list_supplier() {
        ...
        model.addObject("items", e);
        model.addObject("user", get_user());
        model.addObject("filter", new Supplier());
        return model;
    }
...
//    }
    ...
    public String checkout_submit(@Valid @ModelAttribute("vc") VoucherCheckout vc,
        ...
        for (Cart i : cart) {
            ...
            if (i.getQuantity() > i.getI().getQt_avail()) {
                redirectAttributes.addFlashAttribute("message2", i.getI().getName() + " available quantity is " + i.getI().getQt_avail() + ". Please adjust quantity.");
                return "redirect:/cart";
            }
        }
        r.setAmount(total);
        ...
        for (Cart i : cart) {
            ...
            //Added triiger test2 for this
//            Item x = i.getI();
//            x.setQt_avail(x.getQt_avail() - i.getQuantity());
//            x.setSold_qt(x.getSold_qt() + i.getQuantity());
//            userRepository.update_item(x);
        }
...
//        m.addAttribute("items",item);
    ...
    public ModelAndView get_reciept_detail(@PathVariable int id) {
        ...
        Reciept rc = userRepository.get_rc_by_id(id);
        m.addObject("items", rc);
        List<Ritem> p = userRepository.get_rc_items(rc.getReceipt_id());
        for (Ritem r : p) {
            r.setI(userRepository.get_item(r.getItem_id()));
        }
        m.addObject("p", p);
        m.addObject("user",get_user());
        return m;
    ...
    @RequestMapping("adminrc")
    public ModelAndView get_admin_reciept() {
        ModelAndView m = new ModelAndView("/rc/list.jsp");
        m.addObject("items", userRepository.get_reciept());
        return m;
    }

    @RequestMapping("feedback")
    public String get_feedback_list(Model m)
    {
        ...
        m.addAttribute("list",userRepository.get_all_Feedback_by_customer(c.getCustomer_id()));

        return  "/feedback/list.jsp";
    }
@RequestMapping("feedback/add/{rid}/{id}")
public ModelAndView feedback_add(@PathVariable int rid,@PathVariable int id) {
    ModelAndView m = new ModelAndView("/feedback/add.jsp");
    ...
    public String submit_feedback(@Valid @ModelAttribute("f") Feedback f,
                               BindingResult result, ModelMap model) {
        if (result.hasErrors()) {
            return "error.jsp";
        }
        ...
        String username = ((UserDetails) principal).getUsername();
        Customer c = userRepository.get_customer_by_username(username);
        f.setCustomer_id(c.getCustomer_id());

        if (f.getFeedback_id() == 0)
            userRepository.add_Feedback(f);
        return "redirect:/item";
    }

@RequestMapping("update/personal/{id}")
public  ModelAndView update_personal(@PathVariable int id)
{
    ModelAndView m=new ModelAndView("/personal_info_update.jsp");
    Personal_info p=userRepository.get_perosnal_info_by_id(id);
    m.addObject("p",p);
    return  m;
}

    @RequestMapping(value = "save_personal",method = RequestMethod.POST)
    public  String update_personal_submit(@Valid @ModelAttribute("p") Personal_info p)
    {
        userRepository.update_personal_info(p);
        return  "redirect:/item";
    }
    ...
    {
        ModelAndView m=new ModelAndView("/reg.jsp");
    ...
    @RequestMapping(value = "save_register",method = RequestMethod.POST)
    public  String save_register(@Valid @ModelAttribute("c") Customer c)
    {
        User u=c.getU();
        u.setType("Customer");
        u.setRecovery_code("abc");
        ...
        u.setPassword(b.encode(u.getPassword()));
        userRepository.add_User(u);
        Personal_info p=c.getP();
        userRepository.add_perosnal_info(p);
        p=userRepository.get_Person_by_email(p.getEmail());
        c.setPerson_id(p.getPerson_id());
        userRepository.add_Customer(c);

        return  "redirect:/login";
    }

    @GetMapping("users")
    public  String list_user(Model m)
    {
        m.addAttribute("l",userRepository.get_all_Users());
        return "/user.jsp";
    }

    @GetMapping("users/update/{username}")
    public ModelAndView update_user(@PathVariable String  username)
    {
        User u=userRepository.get_User(username);

        ModelAndView m=new ModelAndView("/update_user.jsp");
    ...
    public String del_user(@PathVariable String username)
    {
        userRepository.remove_User(username);
        System.out.println("user deleted sucessfully");
        return "redirect:/users";
    }
    @PostMapping("user")
    public String update(@Valid @ModelAttribute("u") User u)
    {
    ...
    public ModelAndView invoice_form()
    {
        ModelAndView m=new ModelAndView("/invoice.jsp");
        m.addObject("f",new Invoice());
        return  m;
    }
    @PostMapping("/supplier")
    public String invoice_add(@Valid @ModelAttribute("f") Invoice i)
    {
        i.setInvoice_id(userRepository.add_invoice(i));
        userRepository.add_invoice_item(i);


        return "redirect:/item";
    }
    @GetMapping("/invoice")
    public String invoice_list(Model m)
    {
        m.addAttribute("l",userRepository.get_all_Invoice());
        return "/invoice/list.jsp";
    }

    @GetMapping("/invoice/{id}")
    public String invoice_detail(@PathVariable int id, Model m)
    {
        m.addAttribute("l",userRepository.get_Invoice(id));
        m.addAttribute("p",userRepository.get_invoce_item(id));
        return "/invoice/detail.jsp";
    }

    @GetMapping("/invoice/remove/{id}")
    public String invoice_delete(@PathVariable int id, Model m)
    {
        userRepository.remove_Invoice(id);
